    ...
    for (pugi::xml_node mesh_data_node : mesh_node.children()){
        ...
        if (strcmp(mesh_data_node.name(), "source") == 0){
            ...
            if (source_id == mesh_vertex_source_id){
                ...
                if (stride != 3){
                    // Debug::error("Invalid array width for vertex array in %s", filename.c_str());
                } else {
                ...
                if (stride != 3){
                    // Debug::error("Invalid array width for normal array in %s", filename.c_str());
                } else {
                ...
                // coordinates
                if (stride != 2){
                    // Debug::error("Invalid array width for uv array in %s",  filename.c_str());
                } else {
